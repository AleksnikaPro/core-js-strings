...
function removeFirstOccurrences(str, value) {
  const indexOfFirstOccurrence = str.indexOf(value);
  if (indexOfFirstOccurrence !== -1) {
    return (
      str.slice(0, indexOfFirstOccurrence) +
      str.slice(indexOfFirstOccurrence + value.length)
    );
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const indexOfFirstOccurrence = str.lastIndexOf(value);
  if (indexOfFirstOccurrence !== -1) {
    return (
      str.slice(0, indexOfFirstOccurrence) +
      str.slice(indexOfFirstOccurrence + value.length)
...
function isPalindrome(str) {
  const strWithoutSpacesAndSigns = str
    .replace(/[^\w\s]|_/g, '')
    .replace(/\s+/g, '');
  const reversedStr = strWithoutSpacesAndSigns.split('').reverse().join('');
  if (strWithoutSpacesAndSigns.toLowerCase() === reversedStr.toLowerCase()) {
    return true;
  }
  return false;
}
...
function findLongestWord(sentence) {
  return sentence.split(/\s+/).sort((a, b) => b.length - a.length)[0];
}
...
function invertCase(str) {
  if (typeof str === 'string') {
    ...
    for (let i = 0; i < splittedStr.length; i += 1) {
      if (splittedStr[i] === splittedStr[i].toLowerCase()) {
        splittedStr[i] = splittedStr[i].toUpperCase();
      } else if (splittedStr[i] === splittedStr[i].toUpperCase()) {
...
function extractNameFromTemplate(value) {
  const template = String.raw`${value}`;
  return template.slice(7, template.length - 1);
}
...
function encodeToRot13(str) {
  const splittedStr = str.split('');
  const alphabetOne = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const alphabetTwo = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let cipheredStr = '';

  for (let i = 0; i < splittedStr.length; i += 1) {
    if (splittedStr[i] === ' ') {
      cipheredStr += splittedStr[i];
    } else if (!splittedStr[i].match(/[a-zA-Z]/)) {
      cipheredStr += splittedStr[i];
    } else {
      const index = alphabetOne.indexOf(splittedStr[i]);
      cipheredStr += alphabetTwo[index];
    }
  }
  return cipheredStr;
}
...
function getCardId(value) {
  ...
  return arrayOfPlayingCard.findIndex((index) => index === value);
}
